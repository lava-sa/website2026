...
function VideoReviewForm() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Details form — shown after preview */}
      {(mode === "preview" || mode === "uploading") && (
...
function ReviewTabs() {
  ...
  const [tab, setTab] = useState<"write" | "video">(
    params.get("tab") === "video" ? "video" : "write"
  );
    ...
    <main className="min-h-screen bg-surface">
      ...
      <div className="section-container max-w-2xl py-12">
        <div className="bg-white shadow-sm border border-border px-8 py-10 sm:px-10">
          <Suspense fallback={<div className="py-8 text-center text-copy-muted">Loading…</div>}>
            <ReviewTabs />
          </Suspense>
        </div>
        <p className="text-center text-xs text-copy-muted mt-6">
          All submissions are moderated by Anneke before going live · Your details are never shared publicly
        </p>
      </div>
