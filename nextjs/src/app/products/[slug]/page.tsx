...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    const fallbackDescription = isMachine
      ...
      : `Buy ${product.name} from Lava-SA — German quality, nationwide South African delivery. Trusted distributor since 2007.`;
            ...
            <div className="flex flex-col gap-5">
              ...
              {isOnOrder ? (
                <>
                  ...
                  <div className="btn-primary flex flex-col items-center justify-center gap-1 text-base py-4 px-4">
                    <span className="flex items-center gap-2 font-bold"><Phone className="h-5 w-5" /> Call us</span>
                    <PhoneNumbers layout="stacked" linkClassName="text-sm font-semibold hover:underline" annekeLabel />
                  </div>
                </div>
                </>
              ) : (
