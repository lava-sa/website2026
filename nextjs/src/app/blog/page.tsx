...
export default async function BlogPage() {
  const [cms, posts] = await Promise.all([
    getSitePageContent("blog-index"),
    getMergedBlogPosts(),
  ]);

  const featured = posts.filter((p) => p.featured);
  const rest = posts.filter((p) => !p.featured);
  const guidesCta = cms.blocks?.[0];

  return (
    <main className="py-16">
      <div className="section-container">
        <div className="max-w-2xl mb-14">
          {cms.hero.overline && <p className="overline mb-3">{cms.hero.overline}</p>}
          <h1 className="text-5xl font-black text-primary">{cms.hero.heading ?? "Lava Blog"}</h1>
          {cms.hero.subtitle && (
            <p className="mt-4 text-lg text-copy-muted leading-relaxed">{cms.hero.subtitle}</p>
          )}
        </div>
        ...
        <div className="mt-20 bg-primary text-white p-10 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <h2 className="text-2xl font-bold mb-3">
              {guidesCta?.heading ?? "Vacuum Packaging Guides"}
            </h2>
            {guidesCta?.bodyHtml ? (
              <CmsBody html={guidesCta.bodyHtml} className="text-white/70 text-sm leading-relaxed prose prose-invert prose-sm max-w-none" />
            ) : (
