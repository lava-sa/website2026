...
const inputCls =
  "w-full border border-border bg-white px-4 py-2.5 text-sm focus:outline-none focus:border-primary transition-colors";
const inputErrCls =
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="relative space-y-6">
      ...
      <div className="bg-primary/5 border-l-4 border-secondary p-4 text-sm text-copy-muted leading-relaxed">
        ...
        <p>
          Specific, detailed answers (not yes/no) help other customers and give Google and AI
          search engines useful, trustworthy content. Take your time — quality beats brevity.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          <label className="block text-xs font-bold text-primary mb-1.5 uppercase tracking-wide">
            Company / Business{" "}
            <span className="text-copy-muted font-normal normal-case tracking-normal">— optional</span>
          </label>
          <input
            type="text"
            value={form.company}
      ...
      <div>
        ...
        <input
          ...
          placeholder="e.g. Best investment I've made for our butchery"
          className={errors.headline ? inputErrCls : inputCls}
        />
      ...
      <div className="space-y-5 border-t border-border pt-6">
        ...
        {config.questions.map((q, idx) => (
          <div key={q.id}>
            ...
            {q.hint && <p className="text-xs text-copy-muted mb-2">{q.hint}</p>}
            <textarea
              value={form.answers[q.id]}
              onChange={(e) => setAnswer(q.id, e.target.value)}
              rows={q.rows ?? 4}
              placeholder={q.placeholder}
              className={`${errors[q.id] ? inputErrCls : inputCls} resize-none`}
            />
            {errors[q.id] && <p className="text-xs text-red-600 mt-1">{errors[q.id]}</p>}
            <p className="text-[10px] text-copy-muted mt-1">
              {form.answers[q.id].length} characters
              {(q.minLength ?? 30) > 0 && ` (minimum ${q.minLength ?? 30})`}
            </p>
          </div>
        ))}
      </div>
      ...
      <div>
        <label className="flex items-start gap-3 cursor-pointer">
          <input
            ...
            onChange={(e) => {
              setForm((p) => ({ ...p, permission: e.target.checked }));
              setErrors((p) => ({ ...p, permission: "" }));
            }}
            className="mt-0.5 h-4 w-4 accent-primary"
          />
          <span className="text-sm text-copy leading-relaxed">
            I confirm this review is genuine and I give Lava-SA permission to publish it on their
            website.
          </span>
        </label>
      ...
      if (!metaRes.ok) {
        ...
        setErrMsg(
          typeof j?.error === "string"
            ? j.error
            : "Step 3 failed: could not save your video. Try WhatsApp or email us."
        );
        setMode("error");
        return;
      }
    ...
    <div className="space-y-5">
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs text-copy-muted mt-0.5">
            Record on your phone → WhatsApp Anneke on +27 79 512 6771
          </p>
        </div>
      ...
      {mode === "idle" && (
        <div className="bg-primary/5 border-l-4 border-secondary p-4">
          ...
          <p className="text-xs text-copy-muted mt-3">
            Face a window, hold the phone steady, and speak naturally — the same detail as a written
            review helps everyone.
          </p>
        </div>
      )}
      ...
      {mode === "idle" && (
        <div>
          <input
            ...
            className="hidden"
            id={`video-file-input-${variant}`}
          />
          <label
            htmlFor={`video-file-input-${variant}`}
          ...
          >
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-copy-muted mt-1">MP4, MOV, WebM · max 500 MB</p>
            </div>
          </label>
        </div>
      )}
...
export default function ReviewFormClient({
  ...
  return (
    <>
        ...
        <button
          ...
          className={`flex items-center gap-2 px-1 pb-4 mr-8 text-sm font-bold border-b-2 transition-colors ${
            tab === "write"
              ...
              : "border-transparent text-copy-muted hover:text-primary"
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-1 pb-4 text-sm font-bold border-b-2 transition-colors ${
            tab === "video"
              ...
              : "border-transparent text-copy-muted hover:text-secondary"
          }`}
        >
      ...
      {tab === "write" ? (
        <WrittenReviewForm variant={variant} />
      ) : (
        <VideoReviewForm variant={variant} />
      )}
    </>
  );
}
